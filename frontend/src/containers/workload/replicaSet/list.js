...
import ReplicaSetDetail from './detail'
import StatusHandler from 'components/statusHandler'
import { transform, flattenWorkload } from 'utils/transform'
import { req } from 'api'
import overviewAPI from 'api/overview'

export default function ReplicaSetList(props) {
    ...
    useEffect(() => {
        const get = async () => {
            try {
                ...
                const res = await req(overviewAPI.getReplicaSet())
                setData(res)
    ...
    return (
        <StatusHandler status={apiStatus} message={apiMessage}>
            <Switch
                ...
                title={selectedTitle}
                content={<ReplicaSetDetail items={transform(selected)} />}
            >
                <TableComponent
                    column={headCells}
