...
import NetworkPolicyDetail from './detail'
import StatusHandler from 'components/statusHandler'
import { flattenWorkload } from 'utils/transform'
import { req } from 'api'
import overviewAPI from 'api/overview'

export default function NetworkPolicyList(props) {
    ...
    useEffect(() => {
        const get = async () => {
            try {
                ...
                const res = await req(overviewAPI.getNetworkPolicy())
                setData(res)
    ...
    return (
        <StatusHandler status={apiStatus} message={apiMessage}>
            <Switch
                ...
                title={selectedTitle}
                content={<NetworkPolicyDetail items={selected} />}
            >
                <TableComponent
                    column={headCells}
